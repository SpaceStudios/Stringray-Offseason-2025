...
public class AutoAlign {
    public static Command alignToPose(Supplier<Pose2d> poseSupplier, Drive drive) {
        ProfiledPIDController xController = new ProfiledPIDController(10.0, 0, 0, new Constraints(5.0, 10.0));
        ProfiledPIDController yController = new ProfiledPIDController(10.0, 0, 0, new Constraints(5.0, 10.0));
        ProfiledPIDController rotController = new ProfiledPIDController(5.0, 0, 0, new Constraints(5.0, 10.0));
        ...
        return Commands.run(() -> {
            ...
            ChassisSpeeds speeds = new ChassisSpeeds(xController.calculate(currentPose.getX()), yController.calculate(currentPose.getY()), rotController.calculate(currentPose.getRotation().getRadians()));
    ...
    public static IntakeLocation getBestIntake(Drive drive) {
        ...
        if (kPos.relativeTo(closestAlgae).getTranslation().getNorm() < kPos.relativeTo(closestCoral).getTranslation().getNorm()) {
